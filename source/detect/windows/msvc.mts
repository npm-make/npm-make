...
async function detectMsvc11(detect: Detect, installPath: string, version: string) {
    ...
    for await (const subItem of subDir) {
        switch (subItem.name.toUpperCase()) {
            case 'ATLMFC':
                await detect.tryAdd(null, 'MSVC', version, 'includePath', subDir.path + '/' + subItem.name + '/include')
                await detect.tryAdd('X86', 'MSVC', version, 'libraryPath', subDir.path + '/' + subItem.name + '/lib')
                await detect.tryAdd('ARM', 'MSVC', version, 'libraryPath', subDir.path + '/' + subItem.name + '/lib/arm')
                await detect.tryAdd('X64', 'MSVC', version, 'libraryPath', subDir.path + '/' + subItem.name + '/lib/amd64')
                break
            case 'BIN':
                await detectMsvc11Bin(detect, subDir.path + '/' + subItem.name, version, '')
                break
            case 'INCLUDE':
                detect.add(null, 'MSVC', version, 'includePath', subDir.path + '/' + subItem.name)
                break
            case 'LIB':
                detect.add('X86', 'MSVC', version, 'libraryPath', subDir.path + '/' + subItem.name)
                await detect.tryAdd('ARM', 'MSVC', version, 'libraryPath', subDir.path + '/' + subItem.name + '/arm')
                await detect.tryAdd('X64', 'MSVC', version, 'libraryPath', subDir.path + '/' + subItem.name + '/x64')
                break
        }
    }
}
